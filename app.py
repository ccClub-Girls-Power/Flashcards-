...
#查看更多flex message的函數
def send_flex_messages(user_id, flex_messages, offset=0):
    flex_messages_to_send = flex_messages[offset:offset + 10]  # 每次發送10條

    if flex_messages_to_send:
        carousel_flex_message = FlexSendMessage(
            alt_text="Carousel Flex Message",
            contents={
                "type": "carousel",
                "contents": flex_messages_to_send + [generate_see_more_bubble(offset + 10)]
            }
        )
        line_bot_api.reply_message(user_id, carousel_flex_message)
    else:
        # 如果沒有更多 Flex Messages，可以執行其他操作或結束流程
        pass

#See more button
def generate_see_more_bubble(offset):
    return {
        "type": "bubble",
        "body": {
            "type": "box",
...
def handle_postback(event):
    ...
    if postback_data.startswith("offset="):
        try:
            offset = int(postback_data.split("=")[1])
            send_flex_messages(user_id, flex_messages, offset)
        except ValueError:
...
flashcard/flash card"""
    ...
    # 一般查看__單字卡
        ...
        if sheet_url:
            ...
            # 生成 Flex Messages
            flex_messages = [
                generate_flex_message(current_time, word_name, pos_list, chinese_list, example_list, note_list) for
                current_time, word_name, pos_list, chinese_list, example_list, note_list in
                zip(data_lists[0], data_lists[1], data_lists[2], data_lists[3], data_lists[4], data_lists[5])]

            # 發送 Flex Messages
            send_flex_messages(user_id, flex_messages)
